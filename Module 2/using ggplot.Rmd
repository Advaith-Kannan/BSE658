...
```{r}
#displaces the points randomly to allow visualization of overlapping points
#shifts it by some value below width and height parameters, upwards or downwards randomly
...
```{r}
#bins represent the number of bins
ggplot(state_data, aes(x=Positive))  + geom_histogram(bins=10, color="white", fill="steelblue" ) 
```
```{r}
ggplot(state_data, aes(x=Positive))  + geom_boxplot(color="black", fill="steelblue")
```

```{r}
manystates_data = filter(covid_data, State== c('Delhi', 'Karnataka', 'Tamil Nadu', 'Uttar Pradesh'))
ggplot(data = manystates_data, aes(x = factor(State), y = Negative)) + geom_boxplot()
```

```{r}
ggplot(data = manystates_data, aes(x=Negative)) + geom_histogram(bins=10,color="white",fill="steelblue",)
```
